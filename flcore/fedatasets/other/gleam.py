...
class GLEAM(TabularDataset):
    ...
    def __getitem__(self, index):
        inputs = torch.tensor(self.data[index]).float()
...
def fetch_gleam(args, root, test_size):
    ...
    def _munge_and_split(root, test_size):
        ...
        for idx, (path, dirs, files) in enumerate(os.walk(root)):
            if idx == 0: # demographic information
                ...
                client_datasets.append(
                    (
                        GLEAM((train_inputs, train_targets)), 
                        GLEAM((test_inputs, test_targets))
                    )
                )
        return client_datasets


    client_datasets = _munge_and_split(os.path.join(root, 'gleam'), test_size)
