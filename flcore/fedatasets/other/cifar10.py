...
class CIFAR10(VisionDataset):
    ...
    def __init__(self, dataset):
        super(CIFAR10, self).__init__(dataset[0], dataset[1])
...
def create_instance(indices, original_dataset):
    ...
    return CIFAR10((selected_data, selected_targets))

    
        
# helper method to fetch CINIC-10 dataset
def fetch_cifar10(args, root, test_size):
    
    # load CINIC-10 with ImageFolder
    DEFAULT_ARGS = {'root': root, 'train': True, 'download': True,
                    'transform': dataset_transform["CIFAR10"]["train"]}
    raw_train = torchvision.datasets.CIFAR10(**DEFAULT_ARGS)
    DEFAULT_ARGS['train'] = False
    DEFAULT_ARGS['transform'] = dataset_transform["CIFAR10"]["test"]
    raw_test = torchvision.datasets.CIFAR10(**DEFAULT_ARGS)
    ...
    raw_train = CIFAR10((raw_train.data, raw_train.targets))
